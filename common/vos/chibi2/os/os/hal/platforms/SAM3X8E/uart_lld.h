 ...
 */
#if !defined(SAM3X_UART_USE_UART1) || defined(__DOXYGEN__)
...
struct UARTDriver {
    ...
    /* End of the mandatory fields.*/
    uart_t uart;
    uint8_t rxcirc[SAM3X_UART_RX_BUFSIZE + 1];
    uint16_t rxreq;
    Thread *tx_thread;
    Thread *rx_thread;
};
...
#define UART_POLL_END_TX(drv) while(!( ((UARTDriver*)(drv))->uart->UART_SR&UART_SR_ENDTX));
