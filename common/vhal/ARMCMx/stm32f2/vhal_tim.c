...
TIM_TypeDef *const tim_l[TIMNUM]  = {TIM1, TIM2, TIM3, TIM4, TIM5, TIM6, TIM7, TIM8, TIM9, TIM10, TIM11};
...
void vhalIRQ_TIM7(void) {
  vosEnterIsr();
  _timIrq_wrapper(6);
  vosExitIsr();
}

void vhalIRQ_TIM6(void) {
  vosEnterIsr();
  _timIrq_wrapper(5);
  vosExitIsr();
}
...
int vhalInitTIM(void *data) {
  ...
  if (!tim_s_initialized) {
    ...
    vosInstallHandler(tim_irqs[5], vhalIRQ_TIM6);
    vosInstallHandler(tim_irqs[6], vhalIRQ_TIM7);
