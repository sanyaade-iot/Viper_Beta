...
int vhalFlashGetSector(void *addr) {
	(void)addr;
    return (((uint32_t)(addr))-0x8000000)/INTERNAL_FLASH_PAGE_SIZE;
}
...
int vhalFlashErase(void *addr, uint32_t size) {
    ...
    uint32_t stop = from + size;

    FLASH_Status sta = FLASH_COMPLETE;
    FLASH_Unlock();
    FLASH_ClearFlag(FLASH_FLAG_BSY | FLASH_FLAG_EOP | FLASH_FLAG_PGERR | FLASH_FLAG_WRPRTERR);
    uint32_t i;
    for (i = from; i < stop; i += INTERNAL_FLASH_PAGE_SIZE) {
        sta = FLASH_ErasePage(i);
    }
    FLASH_Lock();
    return (sta == FLASH_COMPLETE) ? 2 : 0;;
}
...
void *vhalFlashAlignToSector(void *addr) {
	(void)addr;
	uint32_t pnt = (uint32_t)(addr);
	uint32_t mod =  (pnt)% INTERNAL_FLASH_PAGE_SIZE;
     if (mod) {
        pnt += INTERNAL_FLASH_PAGE_SIZE - (mod);
    }
    return (void*)pnt;
}
