...
int vhalInitADC(void *data) {
	...
	if (!adc_initialized) {
		...
		_adcinfo[0].dmastream = 0;
		_adcinfo[0].dma = 0;
		_adcinfo[0].adc = ADC1;

		//prescaler
		RCC->CFGR |= STM32_ADCPRE;
...
const uint16_t _adc_smp[] = {14, 20, 26, 41, 54, 68, 84, 252};


//TODO: support more than one adc
void adc_dma_callback(uint32_t stream, uint32_t sr) {
	if (sr & DMA_ISR_TCIF1) {
...
int vhalAdcInit(uint32_t adc, vhalAdcConf *conf) {
	...
	if (!_adcinfo[adcid].status) {
		...
		/* activates ADC @ 12 MHz (APB2/DIV6) */
...
int vhalAdcRead(uint32_t adc, vhalAdcCaptureInfo *info) {
	...
	switch (info->capture_mode) {
		case ADC_CAPTURE_SINGLE:
			...
			vosSysLock();
			dma_conf_stream(_adcinfo[adcid].dmastream, (uint32_t)&_adcinfo[adcid].adc->DR, (uint32_t)info->buffer, info->samples * info->npins, DMA_16_BIT, DMA_MEM_PRPH_INCR(1, 0));
			dma_stream_enable(_adcinfo[adcid].dmastream);
			_adcinfo[adcid].adc->CR1   = cg->cr1 | ADC_CR1_SCAN;
			_adcinfo[adcid].adc->CR2 = cg->cr2 | ADC_CR2_CONT  | ADC_CR2_DMA | ADC_CR2_ADON;
