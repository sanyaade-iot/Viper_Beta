...
TEST(ser2) {
	int cloop=0;
	cloop =vhalSerialInit(1,115200,SERIAL_PARITY_NONE, SERIAL_STOP_ONE, SERIAL_BITS_8, VM_RXPIN(1),VM_TXPIN(1));
	TEST_ASSERT("init should return 0",cloop==0);
	
	printf("USB!\n");
	while(cloop!='q'){
		vhalSerialWrite(1,"USB!\n",5);
	
		vhalSerialRead(1,&cloop,1);
		//printf("%i\n",cloop);
		vhalSerialWrite(1,&cloop,1);
	}
