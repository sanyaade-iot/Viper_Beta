...
TEST(test_icu) {
	...
	printf("PWM RET %i\n", vhalPwmStart(MAKE_VPIN(PINCLASS_PWM, 0), TIME_U(100, MICROS), TIME_U(20, MICROS)));
	printf("PWM RET %i\n", vhalPwmStart(MAKE_VPIN(PINCLASS_PWM, 1), TIME_U(100, MICROS), TIME_U(20, MICROS)));
	int i;

	for (i = 2; i < 10; i++) {
		printf("ICU %i RET %i\n", i, vhalIcuStart(MAKE_VPIN(PINCLASS_ICU,i), &conf));
		printf("Done! %i\n", icuflag);
		int i;
